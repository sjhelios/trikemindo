...
if (isset($_GET['RequisitionNumber'])) {
	$RequisitionNumber = $_GET['RequisitionNumber'];
}
elseif (isset($_POST['RequisitionNumber'])) {
	$RequisitionNumber = $_POST['RequisitionNumber'];
}
...
if (isset($RequisitionNumber) AND $RequisitionNumber != '') {
	if (($RequisitionNumber)) {
		echo '<br /><b>' . _('The Order Number entered') . ' <u>' . _('MUST') . '</u> ' . _('be Unique') . '.</b><br />';
		unset($RequisitionNumber);
	/*if (!is_numeric($OrderNumber)) {
		echo '<br /><b>' . _('The Order Number entered') . ' <u>' . _('MUST') . '</u> ' . _('be numeric') . '.</b><br />';
		unset($OrderNumber);*/
	} else {
		echo _('PO Number') . ' - ' . $RequisitionNumber;
	}
} else {
...
if (!isset($RequisitionNumber) or $RequisitionNumber == '') {
	if (isset($SelectedSupplier)) {
	...
	echo '<table class="selection">
			<tr>
				<td>' . _('PO Number') . ': <input type="text" name="RequisitionNumber" 
				/*' . _('Order Number') . ': <input type="text" name="OrderNumber"*/
				autofocus="autofocus" maxlength="8" size="9" />  ' . _('Into Stock Location') . ':
...
if (isset($StockItemsResult)) {
	echo '<table cellpadding="2" class="selection">
		<tr>
			...
			<th class="ascending">' . _('On Hand') . '<br />' . _('(KGs)') . '</th>
			<th class="ascending">' . _('On Hand') . '<br />' . _('(PCKGs)') . '</th>
			<th class="ascending">' . _('Units') . '</th>
			<th class="ascending">' . _('Orders') . '<br />' . _('Outstanding') . '<br />' . _('(KGs)') . '</th>
			<th class="ascending">' . _('Units') . '</th>
		</tr>';
	...
	if (isset($RequisitionNumber) AND $RequisitionNumber != '') {
		$SQL = "SELECT purchorders.orderno,
				...
				AND purchorders.requisitionno='" . $RequisitionNumber . "'
	...
	while ($myrow = DB_fetch_array($PurchOrdersResult)) {
		...
		$ModifyPage = $RootPath . '/PO_Header.php?ModifyOrderNumber=' . $myrow['requisitionno'];
		if ($myrow['status'] == 'Printed') {
			$ReceiveOrder = '<a href="' . $RootPath . '/GoodsReceived.php?PONumber=' . $myrow['requisitionno'] . '">' . _('Receive') . '</a>';
		} else {
			$ReceiveOrder = '';
		}
		if ($myrow['status'] == 'Authorised' AND $myrow['allowprint'] == 1) {
			...
			$PrintPurchOrder = '<a target="_blank" href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $myrow['requisitionno'] . '">' . _('Print') . '</a>';
			...
			/*$PrintPurchOrder = '<a target="_blank" href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $myrow['orderno'] . '&amp;realorderno=' . $myrow['realorderno'] . '&amp;ViewingOnly=2">
				' . _('Print Copy') . '</a>';*/
			$PrintPurchOrder = '<a target="_blank" href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $myrow['requisitionno'] . '&amp;realorderno=' . $myrow['realorderno'] . '&amp;ViewingOnly=2">
				' . _('Print Copy') . '</a>';	
		} else {
		...
		echo '<td><a href="' . $ModifyPage . '">' . $myrow['requisitionno'] . '</a></td>
