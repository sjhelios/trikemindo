...
$NetWeight = $myrow['netweight']; /*adding*/
$GrossWeight = $myrow['grossweight']; /*adding*/
$UnitNetWeight = $myrow['units']; /*adding*/
...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/inventory.png" title="' . _('Inventory') .
	'" alt="" /><b>' . ' ' . $StockID . ' - ' . $myrow['description'] . '</b></p>';
...
if ($Its_A_KitSet_Assembly_Or_Dummy == True){
	...
	$TableHeader = '<tr>
						...
						<th class="ascending">' . _('Qty On Hand (Kg)') . '</th>
						<th class="ascending">' . _('Net Weight (KGs) / ') . '' . _($UnitNetWeight) . '</th>
						<th class="ascending">' . _('Qty On Hand in  ') . '' . _($UnitNetWeight) . '</th>
						<th class="ascending">' . _('Re-Order Level (Kg)') . '</th>
						<th class="ascending">' . _('Demand (Kg)') . '</th>
						<th class="ascending">' . _('In Transit (Kg)') . '</th>
						<th class="ascending">' . _('Available in (Kg)') . '</th>
						<th class="ascending">' . _('Available in  ') . ''._($UnitNetWeight) .'</th>
						<th class="ascending">' . _('Qty ') . ''._($UnitNetWeight) .'' . _(' / Pallette') . '</th>
						<th class="ascending">' . _('Tot. in Pallette') . '</th>
						<th class="ascending">' . _('On Order (Kg)') . '</th>
					</tr>';
}
echo $TableHeader;
$j = 1;
...
while ($myrow=DB_fetch_array($LocStockResult)) {
	...
	if ($Its_A_KitSet_Assembly_Or_Dummy == False){
		...
		if (($InTransitQuantityIn+$InTransitQuantityOut) < 0) {
			...
			$AvailablePck = $Available / $NetWeight; /*add*/
		} else {
		...
		printf('<td class="number">%s</td>
				...
				locale_number_format($NetWeight, $DecimalPlaces),/*add*/
				locale_number_format(($myrow['quantity'] / $NetWeight), $DecimalPlaces),/*add*/
				...
				locale_number_format($AvailablePck, $DecimalPlaces),/*add*/
				locale_number_format($GrossWeight, $DecimalPlaces),/*add*/
				locale_number_format(($AvailablePck / $GrossWeight), $DecimalPlaces),/*add*/
