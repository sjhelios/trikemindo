...
if (isset($RequisitionNumber) AND $RequisitionNumber != '') {
	if (!is_numeric($OrderNumber)) {	
		prnMsg(_('The Order Number entered') . ' <U>' . _('MUST') . '</U> ' . _('be unique'), 'error');
		unset($RequisitionNumber);
	} else {
		echo _('Order Number') . ' - ' . $RequisitionNumber;
	}
} else {
...
if (!isset($RequisitionNumber) or $RequisitionNumber == "") {
	...
	/*echo _('POrder Number') . ': <input class="integer" name="OrderNumber" autofocus="autofocus" maxlength="8" size="9" /> ' . _('Into Stock Location') . ':<select name="StockLocation"> ';
	...
	echo _('POrder Number') . ': <input class="text" name="RequisitionNumber" autofocus="autofocus" maxlength="8" size="9" /> ' . _('Into Stock Location') . ':<select name="StockLocation"> ';
	$sql = "SELECT locations.loccode, locationname FROM locations INNER JOIN locationusers ON locationusers.loccode=locations.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1";
...
//end if stock search results to show
	...
	if (isset($RequisitionNumber) AND $RequisitionNumber != '') {
		$SQL = "SELECT purchorders.orderno,
					...
					WHERE purchorders.requisitionno='" . $RequisitionNumber . "'
						/*purchorders.orderno='" . filter_number_format($OrderNumber) . "'*/
	...
	if (DB_num_rows($PurchOrdersResult) > 0) {
		...
		$TableHeader = '<tr>
							...
							<th class="ascending">' . _('PO Total') . '</th>
		...
		while ($myrow = DB_fetch_array($PurchOrdersResult)) {
			...
			$ViewPurchOrder = $RootPath . '/PO_OrderDetails.php?RequisitionNo=' . $myrow['requisitionno'];
			...
			echo '<td><a href="' . $ViewPurchOrder . '">' . $myrow['requisitionno'] . '</a></td>
