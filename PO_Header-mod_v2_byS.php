...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	$_SESSION['PO' . $identifier]->ShipmentTerms = $_POST['ShipmentTerms'];
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	...
	if ($_SESSION['ExistingOrder']) {
		/*OrderNo*/
		echo _(' Modify Purchase Order Number') . ' ' . $_SESSION['PO' . $identifier]->RequisitionNo;
	}
	...
	echo '
		<tr>
			<th><h4>' . _('Terms & Conditions') . '</h4></th>
		 </tr>
		 <tr><td valign="center">';
	...
	echo '<tr>
			<td>' . _('Insurance by') . ':</td>
			<td><input type="text" name="Insur" size="8" value="' . $_POST['Insur'] . '" /></td> 
		</tr>';
		
	echo '<tr>
			<td>' . _('Country of Origin') . ':</td>;
			<td><input type="text" name="CountryOri" size="15" maxlength="14" value="' . $_POST['CountryOri'] . '" /></td> 
		</tr>';
