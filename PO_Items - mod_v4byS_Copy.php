 ...
 */
	if ($InputError!=1){
		...
		if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
			...
			/* Converting Date Number to Roman section */
	...
	// end of Date Number to Roman section		
		// define Supplier Type [supptype] = 1 -> Import OR = 2 -> Local
			$sqlsupptype='SELECT purchorders.supplierno, suppliers.supptype FROM purchorders LEFT JOIN suppliers ON purchorders.supplierno=suppliers.supplierid';
			$SuppType=$sqlsupptype;
			// end of define

		
		/* start generating PO Number */
	echo '<tr>
			<td>'."PO Number : </td><td>".'';		
			...
			if($_POST['Supptype']==1 && !isset($_GET['ModifyOrderNumber']))
			{
				...
				$$_SESSION['PO' . $identifier]->PoNum = $_POST['PoNumb']; // getting the PO Number
