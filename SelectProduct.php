	...
	if ($_POST['Keywords']) {
		...
		if ($_POST['StockCat'] == 'All') {
			$SQL = "SELECT stockmaster.stockid,
							...
							stockmaster.units,
							SUM(locstock.quantity / stockmaster.netweight) AS qohpck,
							stockmaster.mbflag,
							stockmaster.discontinued,
							stockmaster.decimalplaces,
							stockmaster.grossweight,
							SUM(stockmaster.netweight / 2) AS ntwght /*baca netweight*/
...
if (isset($SearchResult) AND !isset($_POST['Select'])) {
	...
	if ($ListCount > 0) {
		...
		$TableHeader = '<tr>
							...
							<th>' . _('Tot. Qty On Hand (Kg)') . '</th>
							<th>' . _('Net Weight / Pckg') . '</th>
							<th>' . _('Total in Packaging') . '</th>
							<th>' . _('Qty Pckg / Pallete') . '</th>
							<th>' . _('Total in Pallete') . '</th>
		...
		while (($myrow = DB_fetch_array($SearchResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			if ($myrow['mbflag'] == 'D') {
				...
				$netweight = locale_number_format($myrow['ntwght'], $myrow['decimalplaces']);
				$qohpck = locale_number_format($myrow['qohpck'], $myrow['decimalplaces']);
				$grossweight = locale_number_format($myrow['grossweight'], $myrow['decimalplaces']);
				$totgrossweight = locale_number_format(($qohpck / $grossweight)); //, $myrow['decimalplaces']
				/*	$sql = "SELECT stkcatpropid,
						label,
						controltype,
						defaultvalue
		...
		while ($PropertyRow = DB_fetch_array($PropertiesResult)) {
			...
			if (DB_num_rows($PropValResult)==0){
				...
				$PropertyValue= locale_number_format($PropertyValue, $myrow['decimalplaces']); /*====add*/
			}
			if ($myrow['discontinued']==1){
				$ItemStatus = '<p class="bad">' . _('Obsolete') . '</p>';
			} else {
			...
			echo /*'<td>' . $ItemStatus . '</td>*/
				...
				<td class="number">' . $netweight . '' . _(' Kg / ') . '' . $myrow['units'] . '</td>
				<td class="number">' . $qohpck . '' . _(' ') . '' . $myrow['units'] . '</td>
				<td class="number">' . $grossweight . '' . _(' ') . '' . $myrow['units'] . '' . _(' / Pallete') . '</td>
				<td class="number">' . $totgrossweight . '' . _(' Pallete') . '' . $ItemStatus . '</td>
				<td><a target="_blank" href="' . $RootPath . '/StockStatus.php?StockID=' . $myrow['stockid'].'">' . _('  ') . '' . _('View') . '</a></td>
				</tr>';
/*
			<td>' . $ItemStatus . '</td>
			$j++;
